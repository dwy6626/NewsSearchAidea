...
class MrpcProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            guid = "%s-%s" % (set_type, i)
            text_a = line[1]  #test is same
            text_b = line[2]  #test is same
            if set_type != 'test':
                label = line[-1]  #test dose not have label
            else:
                label = '0' 
            examples.append(
...
def main():
    ...
    label_list = processor.get_labels() #stsb is [None]
    num_labels = len(label_list) # len([None]) = 1
